...
Divide by w for projection should be done before coordinates change from (-1,-1)x(1,1) to (0,0)x(1,1). It can thus not be integrated in the \texttt{lightMVP} matrix and should be computed by the shader.
To save computations and GPU memory bandwidth (passing a \texttt{vec3} instead of a \texttt{vec4}), this can also be computed by the vertex shader.

Or in vertex shader !

16 bits is too low to store the depth value and creates some artifacts. Switching to 32 bits did remove these artifacts.
